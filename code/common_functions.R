...
make_surv_func <- function(m){
  ...
  list(
    ...
    H4 = list(
      func  = function(t, k, s) plogis(k - s * qlogis(t / m)),
      npar  = 2,
      init  = c(k = -1, s = 1),
      lower = c(k = -2, s = 0.1),
      upper = c(k = 1, s = 2)
    ...
    S_beta = list(
      func  = function(t, a, b) Rbeta(1 - (t/m), b, a, lower = TRUE),
      npar  = 2,
      init  = c(a = 0.5, b = 1.7),
      lower = c(a = 0.1, b = 1),
      upper = c(a = 1,   b = 5)
...
split_par <- function(par, p, qS, qJ) {
  idx <- 1
...
## conditional pmf induced by S(rho(x;j)) ##
pmf_from_rhoj <- function(S_base, eta_val, y, m, rhoj_fun,
                          surv_par = numeric(0),
                          rhoj_par = numeric(0)) {
  y <- as.numeric(y)
  
  get_rhoj <- function(jval) {
    do.call(
      rhoj_fun,
      c(list(eta_val = eta_val, j = jval, m = m), as.list(rhoj_par))
    )
  }
  ...
  S1 <- do.call(S_base, c(list(t = t1), as.list(surv_par)))
  S2 <- do.call(S_base, c(list(t = t2), as.list(surv_par)))
  
  p <- S1 - S2
  
  idx1 <- (y == 1)
  if (any(idx1)) {
    p[idx1] <- 1 - do.call(S_base, c(list(t = t2[idx1]), as.list(surv_par)))
  }
  
  idxm <- (y == m)
  if (any(idxm)) {
    p[idxm] <- do.call(S_base, c(list(t = t1[idxm]), as.list(surv_par)))
  }
...
fit_one_combo <- function(X, y, m, n,
  ...
  nerho_loglik <- function(par) {
    sp <- split_par(par, p = p, qS = qS, qJ = qJ)
  ...
  se_raw <- if (is.null(V)) rep(NA_real_, length(opt$par)) else sqrt(pmax(diag(V), 0))
  
  sp_hat <- split_par(opt$par, p = p, qS = qS, qJ = qJ)
...
make_final_table <- function(fit, criterion, X) {
  ...
  all_par <- c(beta_par, surv_par, rhoj_par)
